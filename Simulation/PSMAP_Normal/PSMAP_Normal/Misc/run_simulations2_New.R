...
library(argparser)
library(magrittr)
source("PSMAP_Normal_functions_New.R")
...
for(i in 1:n_sims) {
  ...
  results_i = Two_arm_simulation(trt.effect=true.trt.effect, niter=niter, MAP_ESS=ESS, Power_A=ESS, prior.std=prior.std, alpha=alpha,limits=c(0.01,2)) 
