...
plotRwePs <- function(data.withps, overall.inc = FALSE, add.text = TRUE,
                      facet.scales = "free_y", ...) {
  stopifnot(inherits(data.withps,
                     what = get.rwe.class("DWITHPS")));
...
plot.balance.fac <- function(dtaps, v, overall.inc = FALSE) {
...
plot.balance.cont <- function(dtaps, v, nstrata, overall.inc = FALSE, facet.scales = "free_y") {
  cur.d <- NULL;
  for (i in 1:nstrata) {
    cur.sub      <- dtaps[which(i == dtaps[["_strata_"]]),];
...
plotRweBalance <- function(data.withps, overall.inc = FALSE, v.cov = NULL,
