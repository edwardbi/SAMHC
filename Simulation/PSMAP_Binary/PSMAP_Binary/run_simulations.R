...
true.trt.effect=0 #treatment effect in log odds scale
ESS=100
prior.std=0.5
alpha=0.167
n_sims = 10
...
set.seed(seed)
true.trt.effect.prob=1/(1+exp(-true.trt.effect)) #treatment effect in prob. scale
niter = 100000
n.methods=6
result_final=list()
...
for(i in 1:n_sims) {
  ...
  sumresults_i=summarize_results(results_i,theta.true = true.trt.effect.prob,cutoff=0.025,parameter="effect")
